...
from utils.file_manager import append_user_record, read_user_history
...
st.info("系统严格按年份使用对应模型。若用户输入的年份尚未建立年度模型，系统不会用其他年份模型替代，而会先保存数据并提供基础财务、流动性、消费结构和心理账户建议。")
...
if submit:
    ...
    try:
        ...
        record["model_year_used"] = int(year) if ffvi_result is not None else None
        record["is_reference_model"] = False
        record["created_at"] = datetime.now().isoformat(timespec="seconds")
        append_user_record(USER_HISTORY_FILE, record)
